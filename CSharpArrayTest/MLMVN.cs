...
namespace CSharpArrayTest
{
    ...
    {
        // -------------------------- Denoise by Pixels -----------------------------------------------------------------
        public static byte[,] Activation(byte[,] noisyImage,int kernel, string weights, int numberofsectors, int inLayerSize,int hidLayerSize)
        {
            ...
            if(offset == 0)
            {
                Console.WriteLine("value of kernel is not properly set.");
                return null;
            }

            // extend the image
            byte[,] image = new byte[height + offset * 2, width + offset * 2];
            image = TiffFunctions.MirrorImage(noisyImage, height, width, offset);

            
            // pre-instantiate complex 2d-array
            double[,] inputArray = new double[kernel, kernel];
            Complex[,] CinputArray = new Complex[kernel, kernel];
            ...
            for (int Q = offset; Q < height + offset; Q++)
            {
                ...
                {
                    byte[,] src = TiffFunctions.CreateWindow(image, Q, P, kernel, offset);
                    // upcast to double
        ...
        public static byte NeuralNetwork(Complex[,] inputArray,string weightName, int numberofsectors, int inLayerSize,int hidLayerSize)
        {
            /*
             * -------------------------- Used Varianles --------------------------------------------
            ...
            if (hidLayerSize != 0)
            {
                ...
                for (int B = 0; B < hidLayerSize; B++)
                {
                    ...
                    Olary[B] = MLMVN.Neuron(inputIlary, hidWeight);
                } // end for loop B
            ...
            // weighted sum
            Complex z = MLMVN.Neuron(inputOlary, Rweight);
            // get the angle
            double output;
        ...
        // -------------------------- Denoise by Patch -----------------------------------------------------------------
        public static byte[,] fdenoiseNeural(byte[,] noisyIm, int step, string fileName, int layer, int[] networkSize, int numberofsectors)
        {
            /*
            ...
            */
            ...
            Console.WriteLine("Configuring Patch Size...");
            // size of input / output patch
            int patchSz = (int)Math.Sqrt(weights[0].GetLength(1) - 1);
            int patchSzOut = (int)Math.Sqrt(weights[layer - 1].GetLength(0));
            // Size of each sector on unit circle
            
            Console.WriteLine("done.\nInput patch size is: {0}", patchSz);
            Console.WriteLine("Output patch size is: {0}", patchSzOut);
            ...
            if (p_diff > 0)
            {
                ...
                noisyIm = TiffFunctions.MirrorImage(noisyIm, height, width, p_diff);
            ...
            // end if
            #endregion
            Console.Write("Difference of the patche size is: {0}", p_diff);
            Console.WriteLine(", beginning variable initialization...");
            ...
            for (int row = 0; row < range_y.GetLength(0); row ++ ) // for each row
            {
                for (int col = 0; col < range_x.GetLength(0); col ++) // for each column
                {
                    ...
                    // process first layer
                    int ii = 0;
                    byte[,] src = TiffFunctions.CreatePatch(noisyIm, range_y[row], range_x[col], patchSz);
                    // upcast to double
                    Array.Copy(src, inputArray, src.Length);
                    // transformation of inputs into complex plane
                ...
                Console.WriteLine("Patch {0} of {1} done.", row + 1, range_y.Length);
            } // end row for loop
            #region Average
            // Average
        ...
        // -------------------------- Denoise by Patch Method 2 --------------------------------------------------------
        public static byte[,] fdenoiseNeural2(byte[,] noisyIm, int step, string fileName, int layer, int[] networkSize,int numberofsectors)
        {
            /*
            *   noisyIm: an image corrupted by AWG noise
                  * the sliding window stride of the denoising 
        ...
        public static byte[,] fdenoiseNeural3(byte[,] noisyIm, int overlap, string fileName, int layer, int[] networkSize, int numberofsectors)
        {
            /*
             *   noisyIm: an image corrupted by AWG noise
             *        the sliding window stride of the denoising 
        ...
        public static Complex[][,] loadMlmvnWeights(string fileName, int layer, int[] networkSize, int[] inputsPerSample)
        {
            ...
            int pos = 0;
            int ind = 0;
            // this loop is executed for each layer
            for (int ii = 0; ii < layer; ii++)
            {
                // obtain the number of neurons in the layer ii (511 511 511 169) / 170, 512, 512, 512
                int len2 = network[ii].GetLength(0);
                // this loop is executed for each neuron in the layer ii
        ...
        public static void saveMlmvnWeights(string fileName, Complex[][,] network, int[] networkSize)
        {
            ...
            BinaryWriter b = new BinaryWriter(File.OpenWrite(fileName));
        ...
        public static int[,] MLMVN_TEST(string fileNameSamples, int numberOfInputSamples, string fileNameWeights, int layer, int[] networkSize, int[] inputsPerSample, int numberofsectors)
        {
            #region Initialization
            ...
            Console.Write("done.\n");
            Console.Write("Loading Leraning Samples...");
        ...
        public static Complex[][,] MLMVN_Learning(string fileNameSamples, int numberOfInputSamples, string fileNameWeights, int layer, int[] networkSize, double globalThreasholdValue, double localThresholdValue, int numberofsectors, bool randomWeights)
        {
            #region Initialization
            // determine number of samples
            int[] inputsPerSample = new int[layer];
            ...
            Console.WriteLine("Initializing Components");
            // load the samples
            Console.Write("Loading Learning Samples...");
            byte[,] samples = loadLearningSamples(fileNameSamples, numberOfInputSamples, twoInputsPerSample);
            Console.Write("done.\n");
            ...
            while (!finishedLearning)
            {
                ...
                // calculate rmse
                rmse = Math.Sqrt(mse);
                Console.WriteLine("ITERATION {0}, RMSE = {1}", iteration, rmse);
                // Check if learning has converged
                if (rmse <= globalThreasholdValue)
                {
                    finishedLearning = true;
                    Console.WriteLine("Learning Converged!!!");
                }
                // end if
                #endregion
